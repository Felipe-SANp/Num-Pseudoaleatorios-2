class TableDNS {
    ...
    // Encuentra la row y column para el valor más cercano en la tabla
    static search(valor) {
        let differenceMin = Number.MAX_VALUE;
        let row = -1;
        let column = -1;

        // Iterar sobre la tabla para encontrar la mejor aproximación
        for (let i = 0; i < TableDNS.Z_VALUES.length; i++) {
            for (let j = 0; j < TableDNS.Z_VALUES[i].length; j++) {
                const difference = Math.abs(TableDNS.Z_VALUES[i][j] - valor);
